    ...
    <span *ngIf="loading==false">
      <span *ngFor="let order of preOrders; let i = index">
        <ul>
          ...
          <br><li><span>Name:  </span><input  styles="border=20px" [(ngModel)]="preOrders[i].name" required/></li>
          
          <br><li><span>Phone: </span><input styles="border=20px" [(ngModel)]="preOrders[i].phone" required/></li>
          <br><li><span>E-mail:</span><input styles="border=20px" [(ngModel)]="preOrders[i].email" required/></li>
        </ul>
      </span>

      <div>
        <button (click)="goBack()">Back</button>
        <button (click)="preBook()" style="font-family:Microsoft JhengHei">送出</button>
      </div>
    </span>
  `,
  styles: [`
